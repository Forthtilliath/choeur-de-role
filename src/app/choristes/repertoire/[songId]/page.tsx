...
export default async function RepertoireSongPage({
  params,
}: {
  params: Promise<{ songId: string }>;
}) {
  const [user, { songId }] = await Promise.all([getMemberWithSeasons(), params]);
  ...
  return (
    <Main variant="choriste" title="Répertoire">
      ...
      >
        <RepertoireContent
          memberSeasons={user.member_seasons}
          voicePartId={user.voice_part_id}
          songId={songId}
        />
      </Suspense>
    </Main>
