...
export async function getVoiceParts(): Promise<VoicePart[]> {
  ...
  if (error) throw new AppError('DB_ERROR', 'Erreur lors de la récupération des pupitres.');
  return data;
}
...
export async function getChoristerVoiceParts(): Promise<VoicePart[]> {
  ...
  const { data, error } = await supabase
    ...
    .order('order_index');
  if (error) throw new AppError('DB_ERROR', 'Erreur lors de la récupération des pupitres.');
  return data.filter((vp) => !vp.is_admin_only);
}

export async function getAdminVoiceParts(): Promise<VoicePart[]> {
  const supabase = await createServerClient();
  const { data, error } = await supabase
    .from('voice_parts')
    .select('*')
