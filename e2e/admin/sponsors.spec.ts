...
test.describe('Admin — Sponsors / Partenaires', () => {
  ...
  test('le formulaire contient les champs Nom et Site web', async ({ page }) => {
    ...
    await expect(page.locator('h2', { hasText: 'Ajouter un sponsor' })).toBeVisible({
      timeout: 3_000,
    });
  ...
  test('Annuler ferme le formulaire sans créer de sponsor', async ({ page }) => {
    ...
    await expect(page.locator('h2', { hasText: 'Ajouter un sponsor' })).not.toBeVisible({
      timeout: 3_000,
    });
  });

  test('chaque sponsor affiche les boutons Modifier, Activer/Désactiver et Supprimer', async ({
    page,
  }) => {
    const items = page.locator('main .flex.items-center.gap-4.p-4.rounded-xl.border.border-border');
  ...
  test('Modifier ouvre le formulaire pré-rempli avec le nom du sponsor', async ({ page }) => {
    ...
    await items
      ...
      .click();
    await expect(page.locator('h2', { hasText: 'Modifier le sponsor' })).toBeVisible({
      timeout: 3_000,
    });
    ...
    await page.getByRole('button', { name: 'Annuler' }).click();
    await expect(page.locator('h2', { hasText: 'Modifier le sponsor' })).not.toBeVisible({
      timeout: 3_000,
    });
  });
  ...
  test('le formulaire affiche les options Statut (En cours / Passé) et Format (Large / Carré)', async ({
    page,
  }) => {
    await page.getByRole('button', { name: /Ajouter un sponsor/i }).click();
    await expect(page.locator('h2', { hasText: 'Ajouter un sponsor' })).toBeVisible({
