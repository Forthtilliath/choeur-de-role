...
test.describe('Admin — Actualités', () => {
  ...
  test('des actualités ou un message vide sont affichés', async ({ page }) => {
    const items = page
      .locator('main .border.rounded-xl')
      .filter({ hasText: /Brouillon|Publié|Programmée/ });
  ...
  test('cliquer sur "Ajouter" affiche le formulaire de création', async ({ page }) => {
    ...
    await expect(page.locator('h2', { hasText: 'Nouvelle actualité' })).toBeVisible({
      timeout: 3_000,
    });
  ...
  test('le bouton Annuler ferme le formulaire de création', async ({ page }) => {
    ...
    await expect(page.locator('h2', { hasText: 'Nouvelle actualité' })).not.toBeVisible({
      timeout: 3_000,
    });
  });
  ...
  test('Modifier ouvre le formulaire pré-rempli et Annuler le ferme', async ({ page }) => {
    ...
    await items
      ...
      .click();
    await expect(page.locator('h2', { hasText: "Modifier l'actualité" })).toBeVisible({
      timeout: 3_000,
    });
    await expect(page.locator('input[placeholder*="Répétition"]')).not.toHaveValue('');
    await page.getByRole('button', { name: 'Annuler' }).click();
    await expect(page.locator('h2', { hasText: "Modifier l'actualité" })).not.toBeVisible({
      timeout: 3_000,
    });
  });
  ...
  test("Publier/Dépublier bascule le statut de l'actualité", async ({ page }) => {
    ...
    await expect(first.getByRole('button', { name: /Publier|Dépublier/i })).not.toHaveText(
      labelBefore ?? '',
      { timeout: 5_000 },
    );
